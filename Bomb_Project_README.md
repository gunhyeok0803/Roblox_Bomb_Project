...
| 라운드 생존                      | +100   |
| 폭탄 전달 성공                    | +15    |
| 클러치 전달 (위험 구간 60초 이후 = danger>0) | +40 |
| 연속 전달 스트릭 (n회째)             | +5 × n |
| 원거리 투척 (30 studs 이상)        | +20    |
| 능동 버프 사용                    | +10    |
| 폭발로 탈락                      | +0 (감점 없음) |

> **감지 원리**: 클러치는 `RoundManager`가 매 틱 갱신하는 `danger`(60초 이후 >0)로, 원거리는 서버가 계산한 투척자↔피격자 거리로 확정 판정합니다.
> 폭발 시각은 60~90초 랜덤이며 **플레이어에게 숨겨지므로**, "폭발 직전 전달" 같은 **운 기반 보상은 두지 않았습니다**(변별력 왜곡 방지).

**버프 (3~5라운드, 생존자 전원 매 라운드 1개 추첨, `BuffConfig`)** — 총 10종

| id           | 이름     | 분류   | 효과                        | 발동  | 가중치 |
| ------------ | ------ | ---- | ------------------------- | --- | --- |
| `swift`      | 신속     | 지속   | 이동속도 ×1.18                | 자동  | 100 |
| `strongArm`  | 강견     | 지속   | 투척 사거리 ×1.30              | 자동  | 100 |
| `highJump`   | 도약     | 지속   | 점프력 ×1.25                 | 자동  | 100 |
| `blink`      | 순간이동   | 일회성  | 8 studs 대시                | **Q** | 50 |
| `reflect`    | 반사막    | 일회성  | 다음 피격을 던진 사람에게 반사         | 자동  | 18  |
| `repel`      | 밀어내기   | 일회성  | 보유 폭탄을 랜덤 대상에게 강제 전달      | **E** | 18 |
| `reroll`     | 재추첨    | 메타   | 버프를 다시 뽑음                 | 즉시  | 15  |
| `doubleDraw` | 이중 획득  | 메타   | 이번 라운드 버프 2개 지급           | 즉시  | 6   |
| `sluggish`   | 둔화     | 디버프  | 이동속도 ×0.85                | 자동  | 35  |
| `weakArm`    | 약한 팔   | 디버프  | 투척 사거리 ×0.80              | 자동  | 35  |

> 가중치가 클수록 자주 뽑힙니다(흔함). 지속/디버프는 라운드 동안 스탯 배수로 적용되고, 일회성은 충전되어 키(Q/E) 또는 자동으로 발동합니다.
...
- `onThrowRequest()` (서버 권한): 소유자/쿨다운/사거리 검증 → `_findTarget`(투영+반경+벽 레이캐스트) → 반사막 체크 → **점수 처리(전달/클러치/원거리/스트릭)** → 소유권 이전.
...
[클라] 좌클릭 → RequestThrowBomb(aimDir)
  └─ [서버] BombManager.onThrowRequest() (판정 서버 재계산)
       ...
       ├─ ScoreManager.award(전달/클러치/원거리/스트릭)
