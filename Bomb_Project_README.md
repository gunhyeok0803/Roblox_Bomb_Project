...
### 버프 (3~5라운드, **카드 3장 중 택1**, `BuffConfig`) — 오퍼되는 긍정 버프 6종
| id | 이름 | 분류 | 효과 | 발동 |
| --- | --- | --- | --- | --- |
...
| `reroll` 🎲 | 재추첨 | 메타 | *현재 미오퍼(휴면)* | — |
| `doubleDraw` ✖️ | 이중 획득 | 메타 | *현재 미오퍼(휴면)* | — |
| `sluggish` 🐌 | 둔화 | 디버프 | 이동속도 ×0.80 · *현재 미오퍼* | — |
| `weakArm` 🔻 | 약한 팔 | 디버프 | 투척 거리 ×0.72 · *현재 미오퍼* | — |

> **카드 선택 흐름:** 각 플레이어에게 **서로 다른 긍정 버프 3장**을 오퍼 → 하나 선택 시 즉시 적용·UI 닫힘. 8초 초과 시 3장 중 랜덤 자동 확정. **디버프·메타는 오퍼 풀에서 제외**(3장 택1이라 아무도 디버프를 안 고르므로). 되돌리려면 `BuffManager.drawNonMeta` 필터에서 `debuff`/`meta` 제외를 풀면 됨.
>
...
- **BombManager** — 소유권/부착(용접), **서버 권한 투척 판정**(클라는 클릭 월드지점만 전송 → 서버가 **몸통→지점** 방향으로 투영+반경(`HIT_RADIUS`)+벽 레이캐스트), 점수 훅.
